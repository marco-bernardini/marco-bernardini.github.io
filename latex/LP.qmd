...
This page serves as an introduction to local projections, following the lectures by Silvia Miranda Agrippino at the university of Oxford. The content is also based on papers by Jordà.
...
To see how they work, let's start by defining $y_t$, the outcome variable, and $x_t$, the controls. The controls $x_t$ denote a vector of exogenous or pre-determined variables, including lags of the outcome and of the policy intervention, which we denote as $s_t$. In this context, the policy intervention can represent an exogenous shock, a structural shock, or a treatment. Denote by $z_t$ a vector of instruments for $s_t$, provided they are available.  
Define and impulse response as:
...
Some ideas are worth exploring. First, though the linearity assumption can seem restrictive, notice that in fact we are approximating the conditional mean introduced earlier with a different regression model for each horizon $h$, rather than imposing a model that characterizes the full path of $y_t$, $s_t$ etc. Second, usually errors are in fact serially correlated, so inference usually relies on Newey-West typestandard errors. Third, local projections can be easily extended to non-linear models, by including non-linear transformations of the regressors, or by using non-parametric methods. Note however, that under linearity we have the very convenient properties that $\mathcal{R}_{s \rightarrow y}(h,\delta) = -\mathcal{R}_{s \rightarrow y}(h,-\delta)$, that $\mathcal{R}_{s \rightarrow y}(h,\delta) = -\mathcal{R}_{s \rightarrow y}(h,-\delta)$ and finally that $\mathcal{R}_{s \rightarrow y}(h,\delta) = \delta\mathcal{R}_{s \rightarrow y}(h,1) = \delta \beta_h$ which imlications should be pretty obvious.  
Note that another interesting application of LP is the following:

$$
\mathcal{R}_{s \rightarrow y}(h, \delta) \equiv P\left[y_{t+h}=1 \mid s_t=s_0+\delta ; \boldsymbol{x}_t\right]-P\left[y_{t+h}=1 \mid s_t=s_0 ; \boldsymbol{x}_t\right] ; \quad h=0,1, \ldots, H,
$$
...
Suppose first that we can observe the shock $s_t$ directly, and that it is exogenous ($\operatorname{Cov}(s_t, v_{t+h} \mid x_t) = 0$). In this case, we can estimate by OLS the following regression:
...
$$
    \Delta w_t=\sum_{h=0}^{\infty} \Theta_h u_{t-h} ; \quad \Theta_0=I_k ; \quad \Theta_h=\Phi^h
$$
...
If we instead represent the system in levels, we have:
...
The two approaches estimate the same object through different routes.

In the VAR(1), we have shown that
...
We now will show that, in the population, the shock isolated by local projections is the same as the shock isolated by a VAR, up to a normalization. To see this, let us denote by $w_t$ the vector of all variables in the system:
...
We are interested in the response of $y_t$ to a shock to $x_t$. The key point is that both LP and VAR isolate the **same unexpected component** of $x_t$, up to a normalization.
...
y_{t+h}
=
\mu_h+\beta_h^{LP}x_t+\varepsilon_h' r_t+\sum_{\ell=1}^{p}\vartheta_{h,\ell}' w_{t-\ell}+\varpi_{h,t}.
$$
...
Where we have divided by $\sqrt{E(\tilde x_t^2)}$ since VAR shocks are normalized. Thus the VAR shock is exactly the same object as the LP shock, rescaled to have unit variance.  
